...
class RoutesSpec() extends AnyWordSpec with Matchers with ScalatestRouteTest {
  ...
  sealed trait RoutesSpecContext extends Routes {
    ...
    val catDTO: InImageDTO = InImageDTO("cat", "asdqwr", visibility = true)
    ...
    val petsAlbum: Album = Album(Some(birdsID), "Pets")
  ...
  "The server" should {
    ...
    "create new image" in new RoutesSpecContext {
      Post("/images/upload", content = catDTO) ~> route ~> check {
    ...
    "create new album" in new RoutesSpecContext {
      Post("/albums/create", content = Album(None, "Nature")) ~> route ~> check {
        status shouldBe created
      }
    }
    "create image in album" in new RoutesSpecContext {
      Post(s"/albums/$petsID/images/upload", content = catDTO) ~> route ~> check {
        status shouldBe created
      }
    }
